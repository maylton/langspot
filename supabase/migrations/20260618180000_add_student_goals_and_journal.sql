...
create table if not exists public.learning_journal_entries (
  ...
  mood text not null default 'good'
    check (mood in ('great', 'good', 'neutral', 'hard')),
  study_minutes integer not null default 0
    check (study_minutes between 0 and 1440),
  new_words text[] not null default '{}',
  created_at timestamptz not null default now()
);

create index if not exists learning_goals_student_idx
  on public.learning_goals(student_id, status, target_date);

create index if not exists learning_journal_student_idx
  on public.learning_journal_entries(student_id, created_at desc);

alter table public.learning_goals enable row level security;
alter table public.learning_journal_entries enable row level security;

drop policy if exists
  "Students manage their own learning goals"
  on public.learning_goals;

drop policy if exists
  "Teachers view their students learning goals"
  on public.learning_goals;

drop policy if exists
  "Students manage their own learning journal"
  on public.learning_journal_entries;

drop policy if exists
  "Teachers view their students learning journal"
  on public.learning_journal_entries;
