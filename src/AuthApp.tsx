...
function TeacherPortal({ profile, authEmail, onProfileChange, onLogout }: { profile: Profile; authEmail: string; onProfileChange: (profile: Profile) => void; onLogout: () => void }) {
  ...
  const loadTeacherData = async () => {
    ...
    const { data: questionRows, error: questionError } = await supabase.from('question_bank').select('*, question_bank_tasklets(external_id,input_kind,input_text)').order('created_at', { ascending: false });
  ...
  const createMaterial = async (material: MaterialInput) => {
    ...
    if (material.file) {
      ...
      if (!isMaterialUploadFile(material.file) || material.file.size > MAX_UPLOAD_SIZE) throw new Error('Escolha um PDF, DOCX, PPTX ou imagem de até 10 MB.');
      const { error: uploadError } = await supabase.storage.from('materials').upload(storagePath, material.file, { contentType: material.file.type || 'application/octet-stream', upsert: false });
      if (uploadError) throw new Error(`Falha no upload: ${uploadError.message}`);
      const { data: signed } = await supabase.storage.from('materials').createSignedUrl(storagePath, 60 * 60);
      resolvedUrl = signed?.signedUrl ?? '';
    }
    const payload = { teacher_id: profile.id, title: material.title, type: material.file ? materialTypeFromFile(material.file) : material.type, level: material.level, skill: material.skill, url: material.file ? '' : material.url, description: material.description, storage_path: storagePath, file_name: material.file?.name ?? null, file_size: material.file?.size ?? null, mime_type: material.file?.type ?? null, material_source: material.file ? 'upload' : 'link' };
  ...
  return <div className={`authenticated-app ${subscription && !subscriptionActive ? 'subscription-locked' : ''}`}>
    ...
    <App
      ...
      assessmentContent={can.useAssessments() && supabase ? <Suspense fallback={<div className="assessment-loading"><LoaderCircle className="spin" size={24} />Carregando avaliações…</div>}><AssessmentDashboard client={supabase} teacherId={profile.id} students={students.map((student) => ({ id: student.id, name: student.name }))} bank={questionBank.map((question) => ({ bankId: question.id, id: question.externalId ?? question.id, externalId: question.externalId, type: question.type, prompt: question.prompt, options: question.options, answer: question.answer, explanation: question.explanation, cefr: question.level, difficulty: question.difficulty ?? ({ A1: 1.5, 'A1+': 2.25, A2: 3, 'A2+': 4, B1: 5, 'B1+': 5.75, B2: 6.5, 'B2+': 7.25, C1: 8, 'C1+': 8.75, C2: 9.5 } as const)[question.level], skill: question.skill, subskill: question.subskill, taskType: question.taskType, topic: question.topic, genre: question.genre, audience: question.audience, qualityStatus: question.qualityStatus, psychometricStatus: question.psychometricStatus, isPilot: question.sourceOrigin === 'cefr_pilot', sourceMaterial: question.skill === 'reading' ? question.taskletInput : question.sourceMaterial, transcript: question.skill === 'listening' ? question.taskletInput : undefined, transcriptVisibility: question.skill === 'listening' ? 'after_submit' : undefined, rubric: question.rubric, responseConstraints: question.responseConstraints, recordingSeconds: question.type === 'speaking' && question.responseConstraints ? question.responseConstraints.max * (question.responseConstraints.unit === 'minutes' ? 60 : 1) : undefined, primaryEvidence: question.primaryEvidence, taskletId: question.taskletExternalId, restricted: question.restricted }))} /></Suspense> : undefined}
...
function dbQuestionBankToQuestion(row: DbQuestionBankItem): QuestionBankItem {
  return { id: row.id, teacherId: row.teacher_id ?? undefined, externalId: row.external_id ?? undefined, bankVersion: row.bank_version ?? undefined, sourceOrigin: row.source_origin ?? 'teacher', level: row.level as QuestionBankItem['level'], category: row.category as QuestionBankItem['category'], type: row.question_type, prompt: row.prompt, options: row.options ?? [], answer: row.answer ?? '', answerKey: row.answer_key ?? undefined, explanation: row.explanation ?? undefined, skill: row.skill ?? undefined, subskill: row.subskill ?? undefined, difficulty: row.difficulty ?? undefined, taskType: row.task_type ?? undefined, topic: row.topic ?? undefined, genre: row.genre ?? undefined, audience: row.audience, operationalDescriptor: row.operational_descriptor ?? undefined, primaryEvidence: row.primary_evidence ?? undefined, responseConstraints: row.response_constraints ?? undefined, rubric: row.rubric ?? undefined, sourceMaterial: row.source_material ?? undefined, taskletId: row.tasklet_id ?? undefined, taskletExternalId: row.question_bank_tasklets?.external_id, taskletInput: row.question_bank_tasklets?.input_text, taskletInputKind: row.question_bank_tasklets?.input_kind, qualityStatus: row.quality_status ?? 'draft', psychometricStatus: row.psychometric_status ?? undefined, restricted: row.restricted ?? false, usageCount: row.usage_count ?? 0, lastUsedAt: row.last_used_at ?? undefined, createdAt: row.created_at };
}
...
type StudentTab = 'Visão geral' | 'Aulas' | 'Progresso' | 'Materiais' | 'Tarefas' | 'Quiz' | 'Avaliações' | 'Flashcards' | 'Metas' | 'Conquistas' | 'Diário' | 'Perfil' | 'Notificações';
...
const wordCountLabel = (count: number) => `${count} ${count === 1 ? 'palavra' : 'palavras'}`;
...
function StudentAssignments({ assignments, materials, mode = 'tasks', onAssignmentsChange, readOnly = false }: { assignments: DbAssignment[]; materials: DbMaterial[]; mode?: 'tasks' | 'quiz'; onAssignmentsChange: React.Dispatch<React.SetStateAction<DbAssignment[]>>; readOnly?: boolean }) {
  ...
  const submit = async (assignment: DbAssignment) => {
    ...
    if (!text && !file) { setMessage('Escreva sua resposta ou anexe um arquivo antes de enviar.'); return; }
    if (file && (!isStudentSubmissionFile(file) || file.size > MAX_UPLOAD_SIZE)) { setMessage('O anexo precisa ser PDF, DOC, DOCX ou imagem de até 10 MB.'); return; }
    ...
    try {
      if (file) {
        ...
        const { error: uploadError } = await supabase!.storage.from('assignment-submissions').upload(uploadedPath, file, { contentType: file.type || 'application/octet-stream', upsert: false });
        if (uploadError) throw new Error(`Falha no upload: ${uploadError.message}`);
    ...
    <section className="student-panel">
      ...
      <div className="student-assignment-list">{visible.length ? visible.map((assignment) => {
        ...
        return <article key={assignment.id} className={`${overdue ? 'overdue' : ''} ${isOpen ? 'open' : ''}`}>
          ...
          {assignment.assignment_type !== 'interactive' && (assignment.status === 'pending' ? <div className="task-response-box"><div className="task-response-header"><div><label htmlFor={`assignment-${assignment.id}`}>Sua resposta</label></div><span className="task-response-count">{wordCountLabel(countWords(drafts[assignment.id] ?? ''))}</span></div><textarea className="task-response-input" id={`assignment-${assignment.id}`} rows={6} value={drafts[assignment.id] ?? ''} onChange={(event) => setDrafts((current) => ({ ...current, [assignment.id]: event.target.value }))} placeholder="Digite sua resposta em inglês ou descreva o que realizou." lang="en" spellCheck={true} autoComplete="off" /><label className="task-pdf-upload"><span>Anexo <small>(opcional)</small></span><input type="file" accept={STUDENT_SUBMISSION_ACCEPT} onChange={(event) => setFiles((current) => ({ ...current, [assignment.id]: event.target.files?.[0] ?? null }))} />{files[assignment.id] && <em>{files[assignment.id]?.name} · {(((files[assignment.id]?.size ?? 0) / 1024 / 1024).toFixed(1))} MB</em>}</label><div className="task-submit-row"><small>Você pode enviar texto, PDF, Word, imagem ou combinar texto e anexo.</small><button className="student-primary" disabled={sendingId === assignment.id} onClick={() => submit(assignment)}>{sendingId === assignment.id ? <LoaderCircle className="spin" size={15} /> : <Check size={15} />}{sendingId === assignment.id ? 'Enviando...' : 'Enviar tarefa'}</button></div></div> : <InteractiveSubmissionReview assignment={assignment} />)}</div>}
        </article>;
