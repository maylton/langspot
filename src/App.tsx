  ...
  return (
    <div className="app-shell">
      ...
      <aside className={`sidebar ${mobileMenuOpen ? 'mobile-open' : ''}`}>
        ...
        <div className="sidebar-footer"><div className="profile-card">{settings.avatar ? <img className="teacher-avatar" src={settings.avatar} alt={`Avatar de ${settings.teacherName}`} /> : <CircleUserRound size={34} />}<div><strong>{settings.teacherName}</strong><span>Professor</span></div>{onLogout && <button onClick={onLogout} title="Sair da conta" aria-label="Sair da conta"><LogOut size={16} /></button>}</div></div>
      </aside>
