...
export type MaterialType = 'PDF' | 'Documento' | 'Apresentação' | 'Imagem' | 'Vídeo' | 'Áudio' | 'Link' | 'Atividade';
...
export const STUDENT_SUBMISSION_ACCEPT = 'application/pdf,.pdf,.doc,.docx,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document,image/*';
export const MATERIAL_UPLOAD_ACCEPT = 'application/pdf,.pdf,.docx,application/vnd.openxmlformats-officedocument.wordprocessingml.document,.pptx,application/vnd.openxmlformats-officedocument.presentationml.presentation,image/*';
export const MAX_UPLOAD_SIZE = 10 * 1024 * 1024;
export const STUDENT_SUBMISSION_MIME_TYPES = ['application/pdf', 'application/msword', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document'];
export const MATERIAL_UPLOAD_MIME_TYPES = ['application/pdf', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document', 'application/vnd.openxmlformats-officedocument.presentationml.presentation'];
export function isStudentSubmissionFile(file: File) { return STUDENT_SUBMISSION_MIME_TYPES.includes(file.type) || file.type.startsWith('image/') || /\.(pdf|doc|docx|jpe?g|png|webp|gif)$/i.test(file.name); }
export function isMaterialUploadFile(file: File) { return MATERIAL_UPLOAD_MIME_TYPES.includes(file.type) || file.type.startsWith('image/') || /\.(pdf|docx|pptx|jpe?g|png|webp|gif)$/i.test(file.name); }
export function materialTypeFromFile(file: File): MaterialType {
  const name = file.name.toLowerCase();
  if (file.type.startsWith('image/') || /\.(jpe?g|png|webp|gif)$/i.test(name)) return 'Imagem';
  if (file.type === 'application/vnd.openxmlformats-officedocument.presentationml.presentation' || name.endsWith('.pptx')) return 'Apresentação';
  if (file.type === 'application/vnd.openxmlformats-officedocument.wordprocessingml.document' || name.endsWith('.docx')) return 'Documento';
  return 'PDF';
}
...
export type QuestionBankRubricCriterion = { key: string; label: string; maxScore: number; scale?: 'numeric' | 'cefr' };
export type QuestionBankItem = { id: Id; teacherId?: Id; externalId?: string; bankVersion?: string; sourceOrigin?: 'teacher' | 'cefr_pilot'; level: QuestionBankLevel; category: QuestionBankCategory; type: InteractiveQuestionType; prompt: string; options: string[]; answer: string; answerKey?: 'A' | 'B' | 'C' | 'D'; explanation?: string; skill?: 'reading' | 'listening' | 'writing' | 'spoken_production' | 'spoken_interaction' | 'mediation' | 'language_use'; subskill?: string; difficulty?: number; taskType?: string; topic?: string; genre?: string; audience?: 'child' | 'teen' | 'adult' | 'general' | 'teen_adult'; operationalDescriptor?: string; primaryEvidence?: string; responseConstraints?: { min: number; max: number; unit: 'words' | 'seconds' | 'minutes'; label: string }; rubric?: QuestionBankRubricCriterion[]; sourceMaterial?: string; taskletId?: string; taskletExternalId?: string; taskletInput?: string; taskletInputKind?: 'text' | 'audio_script'; qualityStatus?: 'draft' | 'reviewed' | 'approved_for_pilot' | 'pilot_data_collected' | 'item_analysis' | 'approved' | 'pilot' | 'needs_revision' | 'retired'; psychometricStatus?: 'uncalibrated' | 'pilot_data_collected' | 'under_analysis' | 'calibrated'; restricted?: boolean; usageCount?: number; lastUsedAt?: string; createdAt: string };
...
export type PlatformSettings = { teacherName: string; email: string; whatsappPhone: string; schoolName: string; avatar: string; defaultDuration: number; defaultOnlineUrl: string; compactMode: boolean; theme: 'light' | 'dark'; confirmCancellations: boolean };
...
export type StudentCreateInput = { name: string; email: string; whatsappPhone?: string; age?: string; level: string; goal: string; notes: string; status: 'Ativo' | 'Pausado'; firstLessonDate?: string; firstLessonTime?: string; firstLessonDuration?: number; firstLessonTopic?: string; firstLessonOnlineUrl?: string };
...
type View = 'Visão geral' | 'Notificações' | 'Alunos' | 'Aulas' | 'Materiais' | 'Tarefas' | 'Quiz' | 'Avaliações' | 'Financeiro' | 'Progresso' | 'Relatórios' | 'Configurações';
...
const defaultSettings: PlatformSettings = can.useMockData() ? mockSettings : { teacherName: '', email: '', whatsappPhone: '', schoolName: '', avatar: '', defaultDuration: 60, defaultOnlineUrl: '', compactMode: false, theme: 'light', confirmCancellations: true };
...
const navigation: { label: View; icon: typeof LayoutDashboard }[] = [
  ...
  ...(can.useAssessments() ? [{ label: 'Avaliações' as View, icon: ClipboardList }] : []),
...
function App({ onLogout, onInviteStudent, onInviteTeacher, onManageTeachers, onCreateStudentAccount, onOpenCancellationRequests, cancellationRequestCount = 0, initialSettings, onProfileSettingsChange, authenticatedMode = false, accountAccess, assessmentContent, initialStudents, initialSchedule, initialMaterials, initialAssignments, initialQuestionBank, initialPayments, initialNotifications, onMarkNotificationRead, onMarkAllNotificationsRead, onCreateScheduledLesson, onUpdateScheduledLesson, onCancelScheduledLesson, onCompleteScheduledLesson, onCreateLessonRecord, onUpdateLessonRecord, onDeleteLessonRecord, onUpdateStudentSkills, onUpdateStudentProfile, onDeleteStudent, onPreviewStudent, onCreateMaterial, onDeleteMaterial, onAssignMaterial, onCreateAssignment, onDeleteAssignment, onReviewAssignment, onCreateQuestionBankItem, onDeleteQuestionBankItem, onCreatePayment, onUpdatePaymentStatus, onDeletePayment }: { onLogout?: () => void; onInviteStudent?: () => void; onInviteTeacher?: () => void; onManageTeachers?: () => void; onCreateStudentAccount?: (data: StudentCreateInput) => Promise<StudentAccountResult>; onOpenCancellationRequests?: () => void; cancellationRequestCount?: number; initialSettings?: Partial<PlatformSettings>; onProfileSettingsChange?: (settings: PlatformSettings) => void | Promise<void>; authenticatedMode?: boolean; accountAccess?: AccountAccessInfo; assessmentContent?: ReactNode; initialStudents?: Student[]; initialSchedule?: ScheduledLesson[]; onCreateScheduledLesson?: (lesson: ScheduledLessonInput) => Promise<ScheduledLesson>; onUpdateScheduledLesson?: (id: Id, lesson: ScheduledLessonInput) => Promise<ScheduledLesson>; onCancelScheduledLesson?: (id: Id) => Promise<void>; onCompleteScheduledLesson?: (id: Id, record: LessonRecordInput) => Promise<void>; onCreateLessonRecord?: (studentId: Id, record: LessonRecordInput) => Promise<ScheduledLesson>; onUpdateLessonRecord?: (id: Id, record: LessonRecordInput) => Promise<ScheduledLesson>; onDeleteLessonRecord?: (id: Id) => Promise<void>; onUpdateStudentSkills?: (studentId: Id, skills: Record<Skill, number>) => Promise<void>; onUpdateStudentProfile?: (student: Student) => Promise<void>; onDeleteStudent?: (studentId: Id) => Promise<void>; onPreviewStudent?: (student: Student) => void; initialMaterials?: Material[]; onCreateMaterial?: (material: MaterialInput) => Promise<Material>; onDeleteMaterial?: (id: Id) => Promise<void>; onAssignMaterial?: (materialId: Id, studentIds: Id[]) => Promise<void>; initialAssignments?: Assignment[]; initialQuestionBank?: QuestionBankItem[]; onCreateAssignment?: (assignment: AssignmentInput) => Promise<Assignment>; onDeleteAssignment?: (id: Id) => Promise<void>; onReviewAssignment?: (id: Id, feedback: string, grade?: number, textComments?: TextComment[]) => Promise<void>; onCreateQuestionBankItem?: (question: QuestionBankInput) => Promise<QuestionBankItem>; onDeleteQuestionBankItem?: (id: Id) => Promise<void>; initialPayments?: Payment[]; initialNotifications?: AppNotification[]; onMarkNotificationRead?: (id: Id) => Promise<void>; onMarkAllNotificationsRead?: (ids: Id[]) => Promise<void>; onCreatePayment?: (payment: PaymentInput) => Promise<Payment>; onUpdatePaymentStatus?: (id: Id, status: PaymentStatus) => Promise<void>; onDeletePayment?: (id: Id) => Promise<void> }) {
  ...
  const addStudent = async (data: StudentCreateInput) => {
    ...
    const student: Student = { id: studentId, name: data.name, email: data.email, whatsappPhone: data.whatsappPhone, age: data.age, level: data.level, goal: data.goal, status: data.status, nextClass: createdLesson ? formatNextLesson(createdLesson) : '', notes: data.notes, progress: 50, skills: defaultSkills(), lessons: [] };
  ...
  const visibleNavigation = navigation.filter((item) => item.label !== 'Avaliações' || assessmentContent);
  const teacherPrimaryNavigation = visibleNavigation.filter((item) => ['Visão geral', 'Alunos', 'Aulas', 'Quiz'].includes(item.label));
  const teacherMoreNavigation = visibleNavigation.filter((item) => !teacherPrimaryNavigation.some((primary) => primary.label === item.label));
  ...
  return (
    <div className="app-shell">
      ...
      <aside className={`sidebar ${mobileMenuOpen ? 'mobile-open' : ''}`}>
        ...
        <nav className="desktop-nav">{visibleNavigation.map(({ label, icon: Icon }) => <button key={label} className={active === label && !selected ? 'nav-item active' : 'nav-item'} onClick={() => navigateTo(label)}><Icon size={19} /><span>{label}</span>{label === 'Notificações' && unreadNotificationCount > 0 && <b className="nav-badge">{unreadNotificationCount > 9 ? '9+' : unreadNotificationCount}</b>}</button>)}</nav>
...
function MaterialsPage({ materials, onNew, onDelete, onAssign }: { materials: Material[]; onNew: () => void; onDelete: (material: Material) => void; onAssign: (material: Material) => void }) {
  ...
  return <div className="materials-layout">
    ...
    <section className="panel materials-toolbar"><div className="search-box"><Search size={17} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar título, habilidade..." /></div><div className="material-filters"><select value={typeFilter} onChange={(event) => setTypeFilter(event.target.value as MaterialType | 'Todos')}><option>Todos</option><option>PDF</option><option>Documento</option><option>Apresentação</option><option>Imagem</option><option>Vídeo</option><option>Áudio</option><option>Link</option><option>Atividade</option></select><select value={levelFilter} onChange={(event) => setLevelFilter(event.target.value)}><option>Todos</option><option>A1</option><option>A2</option><option>B1</option><option>B2</option><option>C1</option><option>C2</option></select></div></section>
...
function StudentEditModal({ student, onClose, onSave }: { student: Student; onClose: () => void; onSave: (student: Student) => Promise<void> }) {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); setBusy(true); setError(''); const form = new FormData(event.currentTarget); try { await onSave({ ...student, name: String(form.get('name')), whatsappPhone: String(form.get('whatsappPhone') || ''), age: String(form.get('age') || ''), level: String(form.get('level')), goal: String(form.get('goal')), status: String(form.get('status')) as Student['status'], notes: String(form.get('notes')) }); } catch (caught) { setError(caught instanceof Error ? caught.message : 'Não foi possível atualizar o perfil.'); setBusy(false); } };
  return <Modal title={`Editar perfil · ${student.name}`} onClose={onClose}><form className="form-grid" onSubmit={submit}><Field name="name" label="Nome completo" defaultValue={student.name} required /><Field name="email" label="E-mail de acesso" type="email" defaultValue={student.email} disabled /><Field name="whatsappPhone" label="WhatsApp" defaultValue={student.whatsappPhone ?? ''} placeholder="+55 85 99999-9999" /><Field name="age" label="Idade" type="number" min="1" max="120" defaultValue={student.age} /><label>Nível<select name="level" defaultValue={student.level}>{['A1','A2','B1','B2','C1','C2'].map((level) => <option key={level}>{level}</option>)}</select></label><Field name="goal" label="Objetivo" defaultValue={student.goal} required /><label>Status<select name="status" defaultValue={student.status}><option>Ativo</option><option>Pausado</option></select></label><label className="full-field">Observações<textarea name="notes" rows={4} defaultValue={student.notes} /></label>{error && <div className="auth-message full-field">{error}</div>}<div className="form-actions"><button type="button" className="cancel-button" onClick={onClose}>Cancelar</button><button className="primary-button" disabled={busy}>{busy ? 'Salvando...' : 'Salvar alterações'}</button></div></form></Modal>;
}
...
function MaterialModal({ onClose, onSave }: { onClose: () => void; onSave: (material: MaterialInput) => void | Promise<void> }) {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    setError('');
    if (mode === 'upload' && !file) { setError('Selecione um arquivo.'); return; }
    if (file && (!isMaterialUploadFile(file) || file.size > MAX_UPLOAD_SIZE)) { setError('Escolha um PDF, DOCX, PPTX ou imagem de até 10 MB.'); return; }
    const form = new FormData(event.currentTarget);
    setBusy(true);
    try {
      await onSave({ title: String(form.get('title')), type: mode === 'upload' && file ? materialTypeFromFile(file) : String(form.get('type')) as MaterialType, level: String(form.get('level')), skill: String(form.get('skill')) as Skill, url: mode === 'link' ? String(form.get('url')) : '', description: String(form.get('description')), source: mode, file: file ?? undefined });
    } catch (caught) {
      setError(caught instanceof Error ? caught.message : 'Não foi possível salvar o material.');
      setBusy(false);
    }
  };
  return <Modal title="Adicionar material" onClose={onClose}><form className="form-grid" onSubmit={submit}><div className="material-source-tabs full-field"><button type="button" className={mode === 'link' ? 'active' : ''} onClick={() => { setMode('link'); setFile(null); }}><ExternalLink size={16} />Adicionar link</button><button type="button" className={mode === 'upload' ? 'active' : ''} onClick={() => setMode('upload')}><FileText size={16} />Enviar arquivo</button></div><Field name="title" label="Título" placeholder="Ex.: Past Simple Review" required /><label>Tipo<select name="type" disabled={mode === 'upload'} defaultValue={mode === 'upload' ? 'PDF' : 'Link'}><option>PDF</option><option>Documento</option><option>Apresentação</option><option>Imagem</option><option>Vídeo</option><option>Áudio</option><option>Link</option><option>Atividade</option></select></label><label>Nível<select name="level"><option>Todos</option><option>A1</option><option>A2</option><option>B1</option><option>B2</option><option>C1</option><option>C2</option></select></label><label>Habilidade<select name="skill">{skills.map((skill) => <option key={skill}>{skill}</option>)}</select></label>{mode === 'link' ? <Field name="url" label="Link do material" type="url" placeholder="https://..." required /> : <label className="full-field pdf-upload-field"><span>Arquivo</span><input type="file" accept={MATERIAL_UPLOAD_ACCEPT} onChange={(event) => setFile(event.target.files?.[0] ?? null)} required />{file && <small>{file.name} · {(file.size / 1024 / 1024).toFixed(1)} MB</small>}<em>PDF, DOCX, PPTX ou imagem, com tamanho máximo de 10 MB.</em></label>}<label className="full-field">Descrição<textarea name="description" rows={4} placeholder="Como este material pode ser utilizado na aula?" /></label>{error && <div className="auth-message full-field">{error}</div>}<div className="form-actions"><button type="button" className="cancel-button" onClick={onClose}>Cancelar</button><button className="primary-button" type="submit" disabled={busy}><Plus size={17} />{busy ? 'Enviando...' : mode === 'upload' ? 'Enviar arquivo' : 'Adicionar material'}</button></div></form></Modal>;
}
...
function materialIcon(type: MaterialType) { if (type === 'Vídeo') return Video; if (type === 'Áudio') return FileAudio; if (type === 'Atividade') return FileQuestion; if (type === 'Link') return ExternalLink; if (type === 'Imagem') return ImagePlus; return FileText; }
