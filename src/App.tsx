...
function SettingsPage({ settings, authenticatedMode, accountAccess, onSave, counts, onExport, onReset, onInviteTeacher }: { settings: PlatformSettings; authenticatedMode: boolean; accountAccess?: AccountAccessInfo; onSave: (settings: PlatformSettings) => void; counts: { students: number; lessons: number; materials: number }; onExport: () => void; onReset: () => void; onInviteTeacher?: () => void }) {
...
function StudentProfile({ student, onNewLesson, onSkillChange, onEdit, onDelete, onPreview }: { student: Student; onNewLesson: () => void; onSkillChange: (skill: Skill, value: number) => void; onEdit: () => void; onDelete: () => void; onPreview: () => void }) {
  return <div className="profile-layout">
    <section className="panel student-summary"><div className="profile-identity"><div className="avatar large">{initials(student.name)}</div><div><span className="status-badge">{student.status}</span><h2>{student.name}</h2><p>{student.email}</p></div></div><div className="detail-grid"><Detail label="Nível" value={student.level} /><Detail label="Objetivo" value={student.goal} /><Detail label="Idade" value={student.age || '—'} /><Detail label="Próxima aula" value={student.nextClass || 'Não agendada'} /></div><div className="note-box"><strong>Observações</strong><p>{student.notes || 'Nenhuma observação adicionada.'}</p></div><div className="student-profile-actions"><button className="secondary-button compact" onClick={onPreview}><Eye size={16} />Visualizar como aluno</button><button className="secondary-button compact" onClick={onEdit}><Edit3 size={16} />Editar perfil</button><button className="danger-button student-delete" onClick={onDelete}><Trash2 size={16} />Excluir aluno</button></div></section>
