...
export function QuestionRenderer({ client, attemptId, question, value, onChange }: { client: SupabaseClient; attemptId: string; question: AssessmentPresentedQuestion; value: string; onChange: (value: string) => void }) {
  ...
  const target = question.responseConstraints?.label;
  if (question.type === 'writing') return <>{target && <p className="assessment-response-target">Extensão esperada: {target}</p>}<label className="assessment-writing-answer">Sua resposta<textarea value={value} onChange={(event) => onChange(event.target.value)} rows={12} maxLength={50000} spellCheck autoComplete="off" /><span>{value.length.toLocaleString('pt-BR')} / 50.000 caracteres</span></label></>;
  if (question.type === 'mediation') return <><p className="assessment-source-material">{question.sourceMaterial}</p>{target && <p className="assessment-response-target">Extensão esperada: {target}</p>}<label className="assessment-writing-answer">Sua mediação<textarea value={value} onChange={(event) => onChange(event.target.value)} rows={12} maxLength={50000} spellCheck autoComplete="off" /><span>{value.length.toLocaleString('pt-BR')} / 50.000 caracteres</span></label></>;
  if (question.type === 'speaking') return <>{target && <p className="assessment-response-target">Duração esperada: {target}</p>}<SpeakingQuestion client={client} attemptId={attemptId} question={question} value={value} onChange={onChange} /></>;
  ...
  return <>{question.sourceMaterial && <p className="assessment-source-material">{question.sourceMaterial}</p>}<div className="assessment-options">{question.options.map((option) => <label key={option} className={value === option ? 'selected' : ''}><input type="radio" name={question.id} checked={value === option} onChange={() => onChange(option)} />{option}</label>)}</div></>;
}
