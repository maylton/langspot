...
import type { AssessmentDraft, AssessmentDraftSection, AssessmentSectionSkill } from '../types';
import { AssessmentPreview } from './AssessmentPreview';

export type AssessmentBankQuestion = QuestionDefinition & { bankId: string };

const newSection = (): AssessmentDraftSection => ({ id: crypto.randomUUID(), title: 'Nova seção', skill: 'grammar', instructions: '', weight: 1, questions: [] });
...
export function AssessmentEditor({ initialDraft, bank, onBack, onSave, onPublish }: {
  ...
  const addManualQuestion = (sectionId: string, question: QuestionDefinition) => {
    const section = draft.sections.find((item) => item.id === sectionId);
...
function ManualQuestionForm({ onAdd }: { onAdd: (question: QuestionDefinition) => void }) {
  ...
  const submit = (event: FormEvent<HTMLFormElement>) => {
    ...
    onAdd({ id: crypto.randomUUID(), type, prompt, options, answer });
