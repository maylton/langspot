...
export type AssessmentBankQuestion = Omit<AssessmentQuestionSnapshot, 'type'> & { type: QuestionType; bankId: string; restricted?: boolean };
  ...
  return <div className="assessment-editor">
    ...
    {preview ? <AssessmentPreview draft={draft} /> : <>
      <section className="assessment-settings-grid">
        <div className="assessment-settings-heading"><div><span>CONFIGURAÇÃO GERAL</span><h3>Organização do teste</h3></div><p>Defina a estrutura, aplicação e entrega dos resultados.</p></div>
        <label className="assessment-title-field">Título<input value={draft.title} onChange={(event) => setDraft({ ...draft, title: event.target.value })} placeholder="Ex.: Unit 4 Progress Test" /></label>
        ...
        <div className="assessment-randomization-options"><span>Randomização</span><label className="check"><input type="checkbox" checked={draft.randomizeQuestions} onChange={(event) => setDraft({ ...draft, randomizeQuestions: event.target.checked })} />Questões</label><label className="check"><input type="checkbox" checked={draft.randomizeOptions} onChange={(event) => setDraft({ ...draft, randomizeOptions: event.target.checked })} />Alternativas</label></div>
        {draft.assessmentMode === 'adaptive' ? <div className="assessment-adaptive-settings"><div><span>MODO ADAPTATIVO</span><strong>Critérios de progressão</strong></div><label>Ability inicial (1–10)<input type="number" min="1" max="10" step="0.5" value={draft.adaptiveInitialAbility} onChange={(event) => setDraft({ ...draft, adaptiveInitialAbility: Number(event.target.value) })} /></label><label>Mínimo por skill<input type="number" min="1" value={draft.adaptiveMinItems} onChange={(event) => setDraft({ ...draft, adaptiveMinItems: Number(event.target.value) })} /></label><label>Máximo por skill<input type="number" min={draft.adaptiveMinItems} value={draft.adaptiveMaxItems} onChange={(event) => setDraft({ ...draft, adaptiveMaxItems: Number(event.target.value) })} /></label><label>Confiança para encerrar<input type="number" min="0.1" max="0.95" step="0.05" value={draft.adaptiveConfidenceThreshold} onChange={(event) => setDraft({ ...draft, adaptiveConfidenceThreshold: Number(event.target.value) })} /></label></div> : null}
        {draft.framework === 'cefr' && <div className="assessment-cefr-settings"><div><span>METADADOS CEFR</span><strong>Versionamento e regra de decisão</strong></div><label>Form version<input value={draft.formVersion} onChange={(event) => setDraft({ ...draft, formVersion: event.target.value })} /></label><label>Decision rule<input value={draft.decisionRuleVersion} readOnly /></label><p className="rubric-summary">Os thresholds iniciais são um <b>PROVISIONAL INTERNAL STANDARD</b>. O resultado é uma estimativa LangSpot alinhada ao CEFR, não uma certificação oficial.</p></div>}
      </section>
      {draft.framework === 'cefr' && <section className="assessment-settings-grid"><label>Filtrar banco por CEFR<select value={bankLevel} onChange={(event) => setBankLevel(event.target.value)}><option value="all">Todos</option>{['A1','A1+','A2','A2+','B1','B1+','B2','B2+','C1','C1+','C2'].map((level) => <option key={level}>{level}</option>)}</select></label><label>Status<select value={bankStatus} onChange={(event) => setBankStatus(event.target.value)}><option value="all">Todos</option><option value="draft">Draft</option><option value="reviewed">Reviewed</option><option value="approved_for_pilot">Approved for pilot</option><option value="pilot_data_collected">Pilot data collected</option><option value="item_analysis">Item analysis</option><option value="approved">Approved</option><option value="needs_revision">Needs revision</option><option value="retired">Retired</option></select></label><label>Subskill, task, tópico ou gênero<input value={bankSearch} onChange={(event) => setBankSearch(event.target.value)} placeholder="Ex.: inference" /></label></section>}
